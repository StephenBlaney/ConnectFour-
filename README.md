...
```  
# Step 4: Dropping our token

Starting off navigate down to the main game loop. Last time we had the user input a number to drop our token but now that we have our graphic up and running, we want the x position of our mouse click and with pygame we can get this super easy. What we can use is this handy line ```print(event.pos)``` under our mouse conditional statement this tells exactly the coordinates when we click the mouse and is displayed in the console in our IDE. Now after exploring the coordinates you notice that the blank positions all go up by 100 and if we can find a way to round the numbers, we can use the system we used for the command line version of our game. After ```turn == 0``` define posx and get the 0 element of that now this is between 0 and 700. We next define col = posx and divide it by Squaresize and we need a whole int number for this to work, so we floor it and cast it to an int just to be sure. Do the same for player two. You should see in your program that when you click on space a number is placed in the CMD. So, we need to navigate back up to the draw_board method and place a conditional statement between where we drew our rectangle and circles. If our board is empty, we draw black circles else if it has a 1 meaning there’s been a token dropped then we want to draw another circle so copy the other line changing our colour to red which has to be defined and while your there define  yellow for player two, so same stuff just copy another circle. Now to see the changes we need to update pygame at the end of our ```drawboard():``` .

![connectfour(1 8)](https://user-images.githubusercontent.com/22968181/57028767-ddb2e280-6c37-11e9-8c77-c7d71f7271bd.PNG)
